 ...
 */
export const getTotalPrice = (cartItems, extra_cost = {}) => {
  let total = 0;
  if (cartItems) {
  ...
  if (Object.keys(extra_cost).length > 0) {
    Object.keys(extra_cost).forEach((key) => {
      if (key == "promo_discount") additional_cost -= extra_cost[key];
